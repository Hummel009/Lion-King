...
import lionking.common.world.feature.LKWorldGenTreasureMound;
import lionking.common.world.map.LKMapGenOutlandsCaves;
...
public class LKChunkProviderOutlands
					...
					for (int l1 = 0; l1 < 8; ++l1) {
						...
						for (int i2 = 0; i2 < 4; ++i2) {
							...
							for (int k2 = 0; k2 < 4; ++k2) {
								if ((d16 += d15) > 0.0D) {
									byteArray[j2 += s] = (byte) LKMod.pridestone.blockID;
			...
			for (int l = 0; l < 16; ++l) {
				...
				for (int k1 = 127; k1 >= 0; --k1) {
					...
					if (k1 <= rand.nextInt(5)) {
						...
						if (byte3 == 0) {
							j1 = -1;
						} else if (byte3 == (byte) LKMod.pridestone.blockID) {
							if (j1 == -1) {
								if (i1 <= 0) {
									byte1 = 0;
									byte2 = (byte) LKMod.pridestone.blockID;
	...
	public Chunk provideChunk(int i, int j) {
		...
		for (int k = 0; k < blocks.length; k++) {
			if (blocks[k] == (byte) LKMod.pridestone.blockID) {
				metadata[k] = 1;
			} else {
				metadata[k] = 0;
			}
