...
public class LKWorldGenMangoTrees extends WorldGenerator {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
			...
			for (int i2 = i - byte0; i2 <= i + byte0 && flag; i2++) {
				for (int l2 = k - byte0; l2 <= k + byte0 && flag; l2++) {
					if (i1 >= 0 && i1 < 256) {
						...
						if (j3 != 0 && j3 != LKMod.mangoLeaves.blockID) {
							flag = false;
						}
					} else {
						flag = false;
			...
			for (int k3 = i - i3; k3 <= i + i3; k3++) {
				...
				for (int i4 = k - i3; i4 <= k + i3; i4++) {
					...
					if ((Math.abs(l3) != i3 || Math.abs(j4) != i3 || random.nextInt(2) != 0 && j2 != 0) && !Block.opaqueCubeLookup[world.getBlockId(k3, k1, i4)]) {
						setBlockAndMetadata(world, k3, k1, i4, LKMod.mangoLeaves.blockID, 0);
					}
				}
		...
		for (int l1 = 0; l1 < l; l1++) {
			...
			if (k2 == 0 || k2 == LKMod.mangoLeaves.blockID) {
				setBlockAndMetadata(world, i, j + l1, k, LKMod.prideWood.blockID, 2);
			}
		}
