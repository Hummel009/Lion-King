...
public class LKWorldGenTicketBooth extends WorldGenerator {
	public static boolean canGenerateAtPosition(int i, int j, int k) {
		if (!LKLevelData.ticketBoothLocations.isEmpty()) {
			for (Object o : LKLevelData.ticketBoothLocations) {
				...
				if (coords.getDistanceSquared(i, j, k) < LKMod.boothLimit * LKMod.boothLimit) {
					return false;
				}
			}
		}
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		int woolType = LKMod.randomBooths ? random.nextInt(16) : 14;
		int woodType = 0;
		Block stairBlock = Block.stairsWoodOak;
		if (LKMod.randomBooths) {
		...
		for (int j1 = 0; j1 < 5; j1++) {
			for (int k1 = 2; k1 < 6; k1++) {
				if (!(j1 > 0 && j1 < 4 && k1 > 2 && k1 < 5)) {
					world.setBlock(i + 14, j + j1, k + k1, LKMod.lionPortalFrame.blockID, 0, 2);
				}
			}
		}
		...
		if (chest != null) {
			...
			switch (l) {
				case 0:
					suitPart = LKMod.ticketLionHead;
					break;
				case 1:
					suitPart = LKMod.ticketLionSuit;
					break;
				case 2:
					suitPart = LKMod.ticketLionLegs;
					break;
				case 3:
					suitPart = LKMod.ticketLionFeet;
			}
