...
public class LKWorldGenRainforestTrees extends WorldGenerator {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if (world.getBlockId(i, j, k) != 0 && world.getBlockId(i, j, k) != LKMod.forestSapling.blockID) {
			return false;
		}
	...
	private boolean generateNormalTree(World world, Random random, int i, int j, int k, int l) {
		...
		for (int i2 = -1; i2 < 2; i2++) {
			for (int k2 = -1; k2 < 2; k2++) {
				setBlockAndMetadata(world, i + i2, j + l1, k + k2, LKMod.forestLeaves.blockID, 0);
				setBlockAndMetadata(world, i + i2, j + l, k + k2, LKMod.forestLeaves.blockID, 0);
				setBlockAndMetadata(world, i + i2, j + l + 2, k + k2, LKMod.forestLeaves.blockID, 0);
		...
		for (int i3 = -2; i3 < 3; i3++) {
			for (int k3 = -2; k3 < 3; k3++) {
				setBlockAndMetadata(world, i + i3, j + l + 1, k + k3, LKMod.forestLeaves.blockID, 0);
				generateVines(world, random, i + i3, j + l + 1, k + k3);
			}
		}
		for (int j1 = 0; j1 <= l + 1; j1++) {
			setBlockAndMetadata(world, i, j + j1, k, LKMod.prideWood.blockID, 1);
		}
	...
	private boolean generateLargeTree(World world, Random random, int i, int j, int k, int l) {
		...
		for (int i1 = -2; i1 < 3; i1++) {
			for (int k1 = -2; k1 < 3; k1++) {
				if ((i1 == -2 && k1 == -2) || (i1 == 2 && k1 == -2) || (i1 == -2 && k1 == 2) || (i1 == 2 && k1 == 2)) {
					...
					setBlockAndMetadata(world, i + i1, j + l1, k + k1, LKMod.forestLeaves.blockID, 0);
		...
		for (int j1 = 0; j1 < 3; j1++) {
			int i1 = j1 == 2 ? 2 : 1;
			int j2 = j1 == 0 ? l1 - 1 : (j1 == 1 ? l - 1 : l);
			setBlockAndMetadata(world, i - i1, j + j2, k, LKMod.prideWood.blockID, 1);
			setBlockAndMetadata(world, i + i1, j + j2, k, LKMod.prideWood.blockID, 1);
			setBlockAndMetadata(world, i, j + j2, k - i1, LKMod.prideWood.blockID, 1);
			setBlockAndMetadata(world, i, j + j2, k + i1, LKMod.prideWood.blockID, 1);
		}
		for (int j1 = 0; j1 <= l + 1; j1++) {
			setBlockAndMetadata(world, i, j + j1, k, LKMod.prideWood.blockID, 1);
	...
	private boolean isBlockReplaceable(IBlockAccess world, int i, int j, int k) {
		...
		return l == Block.vine.blockID || l == LKMod.forestLeaves.blockID;
	}
}
