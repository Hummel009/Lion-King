...
import lionking.common.world.biome.LKBiomeGenAridSavannah;
import lionking.common.world.biome.LKBiomeGenMountains;
import lionking.common.world.biome.LKBiomeGenWoodedSavannah;
...
public class LKWorldGenTrees extends WorldGenerator {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if (!isBlockReplaceable(world, i, j, k) && world.getBlockId(i, j, k) != LKMod.sapling.blockID) {
			return false;
		}
		...
		for (int j2 = 0; j2 < l + 1; j2++) {
			for (int i1 = i; i1 < i + trunkWidth; i1++) {
				for (int k1 = k; k1 < k + trunkWidth; k1++) {
					if (!isBlockReplaceable(world, i1, j + j2, k1) && world.getBlockId(i1, j + j2, k1) != LKMod.sapling.blockID) {
						flag = false;
					}
				}
			}
		...
		for (int i2 = -3; i2 < 4; i2++) {
			for (int k2 = -3; k2 < 4; k2++) {
				setBlockAndMetadata(world, i + i2, j + l + 3, k + k2, LKMod.leaves.blockID, 0);
			}
		}
		...
		for (int i2 = -2; i2 < 3; i2++) {
			for (int k2 = -2; k2 < 3; k2++) {
				setBlockAndMetadata(world, i + i2, j + l + 4, k + k2, LKMod.leaves.blockID, 0);
			}
		}
		...
		for (int j2 = 0; j2 < l + 1; j2++) {
			for (int i1 = i; i1 < i + trunkWidth; i1++) {
				for (int k1 = k; k1 < k + trunkWidth; k1++) {
					setBlockAndMetadata(world, i1, j + j2, k1, LKMod.prideWood.blockID, 0);
				}
			}
		}

		setBlockAndMetadata(world, i, j + l + 1, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i - 1, j + l + 1, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i + 1, j + l + 1, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i, j + l + 1, k - 1, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i, j + l + 1, k + 1, LKMod.prideWood.blockID, 0);

		setBlockAndMetadata(world, i, j + l + 2, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i - 2, j + l + 2, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i + 2, j + l + 2, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i, j + l + 2, k - 2, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i, j + l + 2, k + 2, LKMod.prideWood.blockID, 0);

		setBlockAndMetadata(world, i, j + l + 3, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i - 2, j + l + 3, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i + 2, j + l + 3, k, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i, j + l + 3, k - 2, LKMod.prideWood.blockID, 0);
		setBlockAndMetadata(world, i, j + l + 3, k + 2, LKMod.prideWood.blockID, 0);
	...
	private void tryCanopyShift(World world, Random random, int i, int j, int k, int length, int direction) {
		if (direction == 0) {
			if (world.isAirBlock(i - 4, j, k - 3) && world.isAirBlock(i - 4, j, k - 2) && world.isAirBlock(i - 4, j, k - 1)
				...
				setBlockAndMetadata(world, i - 3, j, k - 3, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 3, j, k + 3, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 2, j + 1, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 2, j + 1, k + 2, LKMod.leaves.blockID, 0);
				for (int i1 = -2; i1 < 3; i1++) {
					setBlockAndMetadata(world, i - 4, j, k + i1, LKMod.leaves.blockID, 0);
				}
				for (int i1 = -1; i1 < 2; i1++) {
					setBlockAndMetadata(world, i - 3, j + 1, k + i1, LKMod.leaves.blockID, 0);
				}
				setBlockAndMetadata(world, i - 3, j, k, LKMod.prideWood.blockID, 0);
			}
		}
		if (direction == 1) {
			if (world.isAirBlock(i + 4, j, k - 3) && world.isAirBlock(i + 4, j, k - 2) && world.isAirBlock(i + 4, j, k - 1)
		...
		if (direction == 2) {
			if (world.isAirBlock(i - 3, j, k - 4) && world.isAirBlock(i - 2, j, k - 4) && world.isAirBlock(i - 1, j, k - 4)
				...
				setBlockAndMetadata(world, i + 2, j + 1, k - 2, LKMod.leaves.blockID, 0);
				for (int i1 = -2; i1 < 3; i1++) {
					setBlockAndMetadata(world, i + i1, j, k - 4, LKMod.leaves.blockID, 0);
				}
				for (int i1 = -1; i1 < 2; i1++) {
					setBlockAndMetadata(world, i + i1, j + 1, k - 3, LKMod.leaves.blockID, 0);
				}
				setBlockAndMetadata(world, i, j, k - 3, LKMod.prideWood.blockID, 0);
			}
		}
		if (direction == 3) {
			if (world.isAirBlock(i - 3, j, k + 4) && world.isAirBlock(i - 2, j, k + 4) && world.isAirBlock(i - 1, j, k + 4)
	...
	private void tryToBranchAt(World world, Random random, int i, int j, int k, int l) {
			...
			if (world.isAirBlock(i - 1, j + l - 1, k) && world.isAirBlock(i - 1, j + l, k) && world.isAirBlock(i - 2, j + l, k) && world.isAirBlock(i - 3, j + l, k)
				...
				for (int i1 = 1; i1 < 5; i1++) {
					for (int k1 = -1; k1 < 2; k1++) {
						setBlockAndMetadata(world, i - i1, j + l, k + k1, LKMod.leaves.blockID, 0);
					}
				}
				setBlockAndMetadata(world, i - 2, j + l, k, LKMod.prideWood.blockID, 0);
				setBlockAndMetadata(world, i - 3, j + l, k, LKMod.prideWood.blockID, 0);

				setBlockAndMetadata(world, i - 2, j + l, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 3, j + l, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 2, j + l, k + 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 3, j + l, k + 2, LKMod.leaves.blockID, 0);

				setBlockAndMetadata(world, i - 2, j + l + 1, k - 1, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 3, j + l + 1, k - 1, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 2, j + l + 1, k + 1, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 3, j + l + 1, k + 1, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 2, j + l + 1, k, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 3, j + l + 1, k, LKMod.leaves.blockID, 0);
			}
		}
		if (direction == 1) {
			if (world.isAirBlock(i + 1, j + l - 1, k) && world.isAirBlock(i + 1, j + l, k) && world.isAirBlock(i + 2, j + l, k) && world.isAirBlock(i + 3, j + l, k)
		...
		if (direction == 2) {
			if (world.isAirBlock(i, j + l - 1, k - 1) && world.isAirBlock(i, j + l, k - 1) && world.isAirBlock(i, j + l, k - 2) && world.isAirBlock(i, j + l, k - 3)
				...
				setBlockAndMetadata(world, i, j + l, k - 2, LKMod.prideWood.blockID, 0);
				setBlockAndMetadata(world, i, j + l, k - 3, LKMod.prideWood.blockID, 0);

				setBlockAndMetadata(world, i - 2, j + l, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 2, j + l, k - 3, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i + 2, j + l, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i + 2, j + l, k - 3, LKMod.leaves.blockID, 0);

				setBlockAndMetadata(world, i - 1, j + l + 1, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i - 1, j + l + 1, k - 3, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i + 1, j + l + 1, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i + 1, j + l + 1, k - 3, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i, j + l + 1, k - 2, LKMod.leaves.blockID, 0);
				setBlockAndMetadata(world, i, j + l + 1, k - 3, LKMod.leaves.blockID, 0);
			}
		}
		if (direction == 3) {
			if (world.isAirBlock(i, j + l - 1, k + 1) && world.isAirBlock(i, j + l, k + 1) && world.isAirBlock(i, j + l, k + 2) && world.isAirBlock(i, j + l, k + 3)
	...
	private boolean isBlockReplaceable(IBlockAccess world, int i, int j, int k) {
		return world.isAirBlock(i, j, k) || world.getBlockId(i, j, k) == LKMod.leaves.blockID;
	}
}
