...
public class LKEntityRafiki extends EntityCreature implements LKCharacter {
	...
	public void onLivingUpdate() {
		...
		for (int i = -16; i < 17; i++) {
			for (int j = -5; j < 6; j++) {
				for (int k = -16; k < 17; k++) {
					if (worldObj.getBlockId(MathHelper.floor_double(posX) + i, MathHelper.floor_double(posY) + j, MathHelper.floor_double(posZ) + k) == LKMod.flowerVase.blockID) {
						flag = true;
					}
				}
			}
		...
		if (getHasSpokenMagicDust() && canTalkDust() && !getHasGivenMagicDust()) {
			if (!worldObj.isRemote) {
				...
				dropItem(LKMod.lionDust.itemID, 4);
			}
	...
	public boolean interact(EntityPlayer entityplayer) {
		...
		if (getHasInteracted() && canTalk() && itemstack != null) {
			if (itemstack.itemID == LKMod.silver.itemID) {
				if (itemstack.stackSize > 3) {
					itemstack.stackSize -= 3;
					if (!worldObj.isRemote) {
						dropItem(LKMod.rafikiCoin.itemID, 1);
			...
			if (itemstack.itemID == LKMod.fur.itemID && entityplayer.inventory.hasItem(Item.book.itemID)) {
				...
				if (!worldObj.isRemote) {
					...
					dropItem(LKMod.questBook.itemID, 1);
				}
				talkTick = 0;
				return true;
			}
			if (itemstack.itemID == Item.book.itemID && entityplayer.inventory.hasItem(LKMod.fur.itemID)) {
				if (itemstack.stackSize > 1) {
					itemstack.stackSize--;
				} else {
					entityplayer.inventory.setInventorySlotContents(entityplayer.inventory.currentItem, null);
				}
				entityplayer.inventory.consumeInventoryItem(LKMod.fur.itemID);
				if (!worldObj.isRemote) {
					LKIngame.sendMessageToAllPlayers("\u00a7e<Rafiki> \u00a7fHere's another Book of Quests.");
					dropItem(LKMod.questBook.itemID, 1);
				}
		...
		if (!getHasObtainedStick() && canTalk()) {
			if (itemstack != null && itemstack.itemID == LKMod.hyenaBone.itemID && itemstack.stackSize == 64) {
				entityplayer.inventory.setInventorySlotContents(entityplayer.inventory.currentItem, new ItemStack(LKMod.rafikiStick));
				if (!worldObj.isRemote) {
		...
		if (getHasSpokenAboutPortal() && !getHasTakenTermites() && canTalk()) {
			if (itemstack != null && itemstack.itemID == LKMod.termiteDust.itemID && itemstack.stackSize >= 4) {
				entityplayer.inventory.consumeInventoryItem(LKMod.termiteDust.itemID);
				entityplayer.inventory.consumeInventoryItem(LKMod.termiteDust.itemID);
				entityplayer.inventory.consumeInventoryItem(LKMod.termiteDust.itemID);
				entityplayer.inventory.consumeInventoryItem(LKMod.termiteDust.itemID);
				for (int i = 0; i < 7; i++) {
		...
		if (getHasTakenTermites() && !getHasTakenMango() && canTalk()) {
			if (itemstack != null && itemstack.itemID == LKMod.mangoDust.itemID && itemstack.stackSize >= 4) {
				entityplayer.inventory.consumeInventoryItem(LKMod.mangoDust.itemID);
				entityplayer.inventory.consumeInventoryItem(LKMod.mangoDust.itemID);
				entityplayer.inventory.consumeInventoryItem(LKMod.mangoDust.itemID);
				entityplayer.inventory.consumeInventoryItem(LKMod.mangoDust.itemID);
				for (int i = 0; i < 7; i++) {
		...
		if (getHasSpokenAltarUse() && canTalk()) {
			if (itemstack != null && itemstack.itemID == LKMod.mangoDust.itemID) {
				if (entityplayer.inventory.hasItem(LKMod.termiteDust.itemID)) {
					entityplayer.inventory.consumeInventoryItem(LKMod.mangoDust.itemID);
					entityplayer.inventory.consumeInventoryItem(LKMod.termiteDust.itemID);
					if (!worldObj.isRemote) {
						LKIngame.sendMessageToAllPlayers("\u00a7e<Rafiki> \u00a7fHere's some more dust!");
						dropItem(LKMod.lionDust.itemID, 1);
					}
					talkTick = 0;
					return true;
				}
			}
			if (itemstack != null && itemstack.itemID == LKMod.termiteDust.itemID) {
				if (entityplayer.inventory.hasItem(LKMod.mangoDust.itemID)) {
					entityplayer.inventory.consumeInventoryItem(LKMod.mangoDust.itemID);
					entityplayer.inventory.consumeInventoryItem(LKMod.termiteDust.itemID);
					if (!worldObj.isRemote) {
						LKIngame.sendMessageToAllPlayers("\u00a7e<Rafiki> \u00a7fHere's some more dust!");
	...
	public float getBlockPathWeight(int i, int j, int k) {
		return worldObj.getBlockId(i, j - 1, k) == LKMod.rafikiWood.blockID && worldObj.getBlockMetadata(i, j - 1, k) == 2 && worldObj.isAirBlock(i, j, k) ? 20.0F : -999999.0F;
	}
