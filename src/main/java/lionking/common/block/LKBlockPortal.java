...
public class LKBlockPortal extends BlockBreakable {
	...
	public static boolean tryToCreatePortal(World world, int i, int j, int k) {
		int l = 0;
		int i1 = 0;
		if (world.getBlockId(i - 1, j, k) == LKMod.lionPortalFrame.blockID || world.getBlockId(i + 1, j, k) == LKMod.lionPortalFrame.blockID) {
			l = 1;
		}
		if (world.getBlockId(i, j, k - 1) == LKMod.lionPortalFrame.blockID || world.getBlockId(i, j, k + 1) == LKMod.lionPortalFrame.blockID) {
			i1 = 1;
		}
		if (l == i1) {
			return false;
		...
		for (int j1 = -1; j1 <= 2; j1++) {
			for (int l1 = -1; l1 <= 3; l1++) {
				...
				if (flag) {
					if (j2 != LKMod.lionPortalFrame.blockID) {
						return false;
					}
					continue;
				}
		...
		for (int k1 = 0; k1 < 2; k1++) {
			for (int i2 = 0; i2 < 3; i2++) {
				world.setBlock(i + l * k1, j + i2, k + i1 * k1, LKMod.lionPortal.blockID, 0, 2);
			}
	...
	public void onNeighborBlockChange(World world, int i, int j, int k, int l) {
		...
		if (world.getBlockId(i, k1 - 1, k) != LKMod.lionPortalFrame.blockID) {
		...
		if (l1 != 3 || world.getBlockId(i, k1 + l1, k) != LKMod.lionPortalFrame.blockID) {
			world.setBlockToAir(i, j, k);
			return;
		}
		...
		if ((world.getBlockId(i + i1, j, k + j1) != LKMod.lionPortalFrame.blockID || world.getBlockId(i - i1, j, k - j1) != blockID) && (world.getBlockId(i - i1, j, k - j1) != LKMod.lionPortalFrame.blockID || world.getBlockId(i + i1, j, k + j1) != blockID)) {
			world.setBlockToAir(i, j, k);
		}
	}
	...
	public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity) {
		if (entity.ridingEntity == null && entity.riddenByEntity == null) {
			if (entity instanceof EntityPlayer) {
				...
				LKMod.proxy.setInPrideLandsPortal(entityplayer);
			}
		}
	}
	...
	public void randomDisplayTick(World world, int i, int j, int k, Random random) {
		...
		for (int l = 0; l < 4; l++) {
			...
			LKMod.proxy.spawnParticle("prideLandsPortal", d, d1, d2, d3, d4, d5);
		}
	}
